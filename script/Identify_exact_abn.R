...
Loglikelihood_Calculation_abn<-function(dataset){
  
  #Learning Bayesian Network using exact search (abn package)
  #creating cache for the network, for maximum p arents, for 11 variables
  cached_network<-buildscorecache(data.df=as.data.frame(dataset),
                                  data.dists=data_distribution, max.parents = parents_maximum)
  #most probable and fitting function from abn
  mp_dag<-mostprobable(score.cache = cached_network,verbose = F)
  dag_11vars<-fitabn(object=mp_dag,dag.banned=banned_matrix,dag.retained=retained_matrix,create.graph = F)
  return(dag_11vars$mlik)
}



Identify_Positions_abn<-function(data,distribution,banned,retained,max_parents,k,n_iteration){
  
  #data=gamelog_discrete
  dataset=data
  n=dim(dataset)[1]
  data_distribution<<-distribution
  banned_matrix<<-banned
  retained_matrix<<-retained
  parents_maximum<<-max_parents
  ...
  repeat{                  #L9
    ...
    loglikelihood_D_current_nonzero_deltas=sum(sapply(D_current,Loglikelihood_Calculation_abn))
    ...
    loglikelihood_D_proposal_nonzero_deltas=sum(sapply(D_proposal,Loglikelihood_Calculation_abn))
    #  for (i in 1:n2) {
