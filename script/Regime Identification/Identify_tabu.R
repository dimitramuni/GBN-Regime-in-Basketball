...
Loglikelihood_Calculation_tabu<-function(dataset){
  
  #Learning Bayesian Network using Tabu algorithm
  ...
  bn<-tabu(dataset,blacklist = blacklisted_arcs1)
...
Identify_Positions_tabu<-function(data,k,n_iteration){
  ...
  repeat{                  #L9
    ...
    loglikelihood_D_current_nonzero_deltas=sum(sapply(D_current,Loglikelihood_Calculation_tabu))
    ...
    loglikelihood_D_proposal_nonzero_deltas=sum(sapply(D_proposal,Loglikelihood_Calculation_tabu))
    #  for (i in 1:n2) {
