...
bn_discrete$Tm=as.factor(chicago_9798$Tm.x)
#str(bn_discrete)
...
for (i in 1:dim(bn_discrete)[1]) {
  #for (j in team_points) {
    ...
    bnn_net<-setEvidence(compile(fitted_grain),nodes = colnames(bn_discrete)[-5],
...
bn_to_discretise_attempts_steal_block$Tm<-bn_to_discretise_attempts_steal_block$Tm.x
